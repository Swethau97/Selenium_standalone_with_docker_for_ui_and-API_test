...
from conftest import SingletonDriverInstance
...
def create_user_fixture():
    ...
    username, password = generate_random_username(),"Password1"
    # Create user via API
    api_requests.create_user(username, "Password1!")
    return username, password




#pt@pytest.mark.usefixtures("driver")
...
def test_compare_books_via_ui_and_api(driver):
    ...
    for i in range (len(Books_from_ui)):
        ...
        assert Books_from_ui[i]["author"] == Books_from_api["books"][i]["author"], \
            f"The assertion failed for author: {Books_from_ui[i]['author']} vs {Books_from_api['books'][i]['author']}"

        assert Books_from_ui[i]["title"] == Books_from_api["books"][i]["title"], \
            f"The assertion failed for title: {Books_from_ui[i]['title']} vs {Books_from_api['books'][i]['title']}"

        assert Books_from_ui[i]["publisher"] == Books_from_api["books"][i]["publisher"], \
            f"The assertion failed for publisher: {Books_from_ui[i]['publisher']} vs {Books_from_api['books'][i]['publisher']}"
